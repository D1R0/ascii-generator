...
class Cell {
    ...
    draw(ctx) {
        ctx.font = `${this.size}px bold monospace`;
...
class AsciiEffect {
    ...
    #width
    #height
    constructor(ctx, width, height, image1) {
        this.#ctx = ctx
        this.#width = width
        this.#height = height
    ...
    scanImage(cellSize) {
        ...
        for (let y = 0; y < this.#pixels.height; y += cellSize) {
            for (let x = 0; x < width; x += cellSize) {
                ...
                if (alpha > 200 && total > 180) {
                    const symbol = this.convertToSymbol(total / 3);
...
function handlerScale() {
    asciiImage(src, inputScale.value)

}
function asciiImage(src, imgSizeMultiplier) {
    const image1 = new Image();
    image1.src = src
    image1.onload = function initialize() {
        canvas.width = image1.width * imgSizeMultiplier;
        canvas.height = image1.height * imgSizeMultiplier;
        effect = new AsciiEffect(ctx, image1.width * imgSizeMultiplier, image1.height * imgSizeMultiplier, image1)
        effect.draw(6)
    }
}
let multiplie = 1
